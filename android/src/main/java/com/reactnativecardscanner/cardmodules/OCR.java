...
class OCR {
  ...
  private CardScanned runModel(Bitmap image) {
    findFour.classifyFrame(image);
		...
		CardScanned cardScanned = recognizeNumbers.number(recognizedDigitsModel, lines);

		if (cardScanned.number == null) {
      ArrayList<ArrayList<DetectedBox>> verticalLines = postDetection.verticalNumbers();
      cardScanned = recognizeNumbers.number(recognizedDigitsModel, verticalLines);
	...
	synchronized CardScanned predict(Bitmap image, Context context) {
